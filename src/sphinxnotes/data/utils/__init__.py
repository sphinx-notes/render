...
class Report(nodes.system_message):
    ...
    def __init__(
        ...
        super().__init__(
            title + ':', type=level, level=2, *children, **attributes
        )
        self.log(title)
    ...
    def problematic(self, inliner: Inliner | tuple[nodes.document, nodes.Element]) -> nodes.problematic:
...
def problematic(msg: nodes.system_message, inliner: Inliner | tuple[nodes.document, nodes.Element]) -> nodes.problematic:
