...
from .renderer import Renderer, Host, ParseHost
...
def render(pending: pending_data, host: Host) -> rendered_data:
    ...
    report = Report('Render Debug Report', 'DEBUG')
    ...
    except Exception as e: # TODO: what excetpion?
    ...
    try:
        ...
        report.text('Failed to render markup text '
    ...
    report.text(f'Systemd messages:')
    for msg in msgs:
        report.report(msg)
...
def replace(pending: pending_data, rendered: rendered_data) -> None:
...
class BaseDataDefiner(ABC):
    ...
    def build_pending_node(
        self, data: PendingData | ParsedData | dict[str, Any],
        tmpl: str, phase: Phase,
    ) -> pending_data:
